...
public class InventoryRepositoryImpl extends BaseRepositoryImpl<Inventory, Long> implements InventoryRepository {
    ...
    public List<Inventory> findByProductOrderByCreatedDateAsc(com.tulip.host.domain.ProductCatalog product) {
        return jpaQueryFactory.selectFrom(INVENTORY).where(INVENTORY.product().eq(product)).orderBy(INVENTORY.createdDate.asc()).fetch();
    }

    @Override
    public List<Inventory> findByProductAndActiveTrue(ProductCatalog product) {
        return jpaQueryFactory.selectFrom(INVENTORY).where(INVENTORY.product().eq(product).and(INVENTORY.active.eq(true))).fetch();
    }

    @Override
    public List<Inventory> findAllByProduct_IdOrderByCreatedDateDesc(Long productId) {
        return jpaQueryFactory
            .selectFrom(INVENTORY)
            .innerJoin(INVENTORY.product(), PRODUCT_CATALOG)
            .fetchJoin()
            .leftJoin(PRODUCT_CATALOG.std(), CLASS_DETAIL)
            .fetchJoin()
            .where(PRODUCT_CATALOG.id.eq(productId))
            .orderBy(INVENTORY.createdDate.desc())
            .fetch();
    }
    ...
    public Map<ProductCatalog, List<Inventory>> findLatestInventoryByProductCatalogGrouped() {
        ...
        return inventories.stream().collect(Collectors.groupingBy(Inventory::getProduct));
    }
}
