...
public class LeaveStateConfig extends StateMachineConfigurerAdapter<LeaveStatus, LeaveEvents> {
    ...
    public void configure(StateMachineConfigurationConfigurer<LeaveStatus, LeaveEvents> config) throws Exception {
        config.withConfiguration().listener(listener()).and().withPersistence().runtimePersister(stateMachineRuntimePersister);
    }
    ...
    private StateMachineListener<LeaveStatus, LeaveEvents> listener() {
        return new StateMachineListenerAdapter<LeaveStatus, LeaveEvents>() {
            ...
            public void stateContext(StateContext<LeaveStatus, LeaveEvents> stateContext) {
                String machineId = stateContext.getStateMachine().getId();
                if (stateContext.getStage().equals(StateContext.Stage.EVENT_NOT_ACCEPTED)) {
                    ...
                    stateAuditService.recordFailedTransition(
                        machineId,
                        ofNullableState(stateContext.getSource()),
                ...
                if (stateContext.getStage().equals(StateContext.Stage.STATE_CHANGED)) {
                    stateAuditService.recordTransition(
                        ...
                        stateContext.getEvent() != null ? stateContext.getEvent().name() : "",
                        true,
    ...
    public void configure(StateMachineStateConfigurer<LeaveStatus, LeaveEvents> states) throws Exception {
        states.withStates().initial(LeaveStatus.PENDING).states(EnumSet.allOf(LeaveStatus.class));
    }
    ...
    private Action<LeaveStatus, LeaveEvents> create() {
        return context -> {
            try {
                ApplyLeaveVM leaveVm = context.getMessageHeaders().get("leaveVm", ApplyLeaveVM.class);
                if (leaveVm == null) {
                ...
                UserRoleEnum role = employee.getGroup().getAuthority().equals(UserRoleEnum.PRINCIPAL.getValue())
                    ? UserRoleEnum.ADMIN
                ...
                if (leaveVm.getTid() != null && !leaveVm.getTid().isEmpty()) {
                    ...
                    context
                        .getStateMachine()
                        .sendEvent(
                            org.springframework.messaging.support.MessageBuilder.withPayload(LeaveEvents.APPROVE)
    ...
    private void sendEmail(EmployeeLeave leave, String templatePath, String action) {
        try {
            ...
            Map<String, Object> model = Map.of("user", employee, "leave", leave, "employeeName", employee.getName(), "action", action);
            String[] ccEmails = employeeService.getCCEmails(employee.getGroup().getAuthority());
    ...
    private Action<LeaveStatus, LeaveEvents> action() {
        return context -> {
            try {
                Long leaveId = context.getMessageHeaders().get("leaveId", Long.class);
                String comments = context.getMessageHeaders().get("comments", String.class);
                LeaveEvents event = context.getEvent();
                Boolean autoApproved = context.getMessageHeaders().get("autoApproved", Boolean.class);
                log.info("Leave {} leaveId={}", event.name(), leaveId);
                employeeLeaveService.updateStatus(
                    leaveId,
                    event.equals(LeaveEvents.APPROVE) ? LeaveStatus.APPROVED : LeaveStatus.REJECTED,
                    comments
                );
                String machineId = context.getStateMachine().getId();
