...
public interface InventoryMapper {
    ...
    default InventoryItemDTO toAggregatedInventoryItemDTO(ProductCatalog productCatalog, List<Inventory> inventories) {
        ...
        int totalPurchasedQty = inventories.stream().mapToInt(Inventory::getQty).sum();

        int totalUsedQty = inventories
            .stream()
    ...
    default Double calculateHighestInventoryPrice(List<Inventory> inventories) {
        double maxPrice = inventories.stream().filter(Inventory::getActive).mapToDouble(Inventory::getMrp).max().orElse(0.0);
